...
const Navigation = () => {
  ...
  const navItems = [
    ...
    { href: '#event-areas', label: t('nav.eventAreas', 'Event Areas') },
              ...
              <div className="glass-card p-4 space-y-3">
                {navItems.map((item) => (
                  <a
                    key={item.href}
                    href={item.href}
                    className="block py-2 px-4 text-sm font-medium font-marykate text-foreground/80 hover:text-primary hover:bg-muted/50 rounded-lg transition-colors"
